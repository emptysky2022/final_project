...
function loadReadData(bno){
	...
	$.getJSON("/sample/list/" + bno, function(result){
		...
		str += '    <label>번호</label><input type="text" name="bno" value=' + result[0].bno + ' readonly></div>';
		str += '  <div class="boardListRE_title">';
		str += '    <label>제목</label><input type="text" name="title" value=' + result[0].title + ' readonly></div>';
		str += '  <div class="boardListRE_content">'
		str += '    <label>내용</label><textarea rows="5" name="content" readonly>' + result[0].content + '</textarea></div>';
		...
		str += '         <b>' + result[1].nickname + '님 리뷰를 남겨</b><br>'
